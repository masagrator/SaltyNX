...
void argvSetup(void)
{
    ...
    if (envIsNso()) {
        ...
        rc = svcQueryMemory(&meminfo, &pageinfo, (u32)argdata);
        ...
        if ((u32)argdata < (u32)meminfo.addr) return;
        if (((u32)argdata - (u32)meminfo.addr) + argdata_allocsize > (u32)meminfo.size) return;
