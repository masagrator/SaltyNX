...
typedef struct {
    ...
    u64 NumRecv; // B
    u64 NumExch; // W
    u64 Buffers[IPC_MAX_BUFFERS];
    u64 BufferSizes[IPC_MAX_BUFFERS];
    BufferType BufferTypes[IPC_MAX_BUFFERS];

    u64 NumStaticIn;  // X
    u64 NumStaticOut; // C
    u64 Statics[IPC_MAX_BUFFERS];
    u64 StaticSizes[IPC_MAX_BUFFERS];
...
static inline void ipcAddSendSmart(IpcCommand* cmd, size_t pointer_buffer_size, const void* buffer, size_t size, u8 index) {
    if (pointer_buffer_size != 0 && size <= pointer_buffer_size) {
        ipcAddSendBuffer(cmd, 0, 0, BufferType_Normal);
        ipcAddSendStatic(cmd, (u64*)buffer, size, index);
    } else {
        ipcAddSendBuffer(cmd, (u64*)buffer, size, BufferType_Normal);
        ipcAddSendStatic(cmd, 0, 0, index);
    }
}
...
static inline void ipcAddRecvSmart(IpcCommand* cmd, size_t pointer_buffer_size, void* buffer, size_t size, u8 index) {
    if (pointer_buffer_size != 0 && size <= pointer_buffer_size) {
        ipcAddRecvBuffer(cmd, 0, 0, BufferType_Normal);
        ipcAddRecvStatic(cmd, (u64*)buffer, size, index);
    } else {
        ipcAddRecvBuffer(cmd, (u64*)buffer, size, BufferType_Normal);
        ipcAddRecvStatic(cmd, 0, 0, index);
    }
}
...
static inline void* ipcPrepareHeader(IpcCommand* cmd, size_t sizeof_raw) {
    ...
    *buf++ = IpcCommandType_Request | (cmd->NumStaticIn << 16) | (cmd->NumSend << 20) | (cmd->NumRecv << 24) | (cmd->NumExch << 28);
    ...
    for (i=0; i<cmd->NumStaticIn; i++, buf+=2) {
        ...
        desc->Packed = cmd->StaticIndices[i] | (cmd->StaticSizes[i] << 16) |
            ((((u64)ptr >> 32) & 15) << 12) | ((((u64)ptr >> 36) & 15) << 6);
    }
    ...
    *fill_in_size_later |= raw_size;
    ...
    for (i=0; i<cmd->NumStaticOut; i++, buf+=2) {
        ...
        desc->Addr = ptr;
        desc->Packed = ((u64)ptr >> 32) | (cmd->StaticSizes[off] << 16);
    }
...
static inline Result ipcParse(IpcParsedCommand* r) {
    ...
    r->Raw = (u32*)(((uintptr_t)(buf + num_bufs*3) + 15) &~ 15);
    r->RawWithoutPadding = (u32*)((uintptr_t)(buf + num_bufs*3));
...
static inline Result ipcParseDomainRequest(IpcParsedCommand* r) {
    ...
    r->Raw = (u32*)(((uintptr_t) r->Raw) + sizeof(DomainMessageHeader));
...
static inline Result ipcParseDomainResponse(IpcParsedCommand* r, size_t sizeof_raw) {
    ...
    r->Raw = (u32*)(((uintptr_t) r->Raw) + sizeof(DomainResponseHeader));
